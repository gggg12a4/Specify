        ...
        <div class="dialog" @click.stop>
          ...
          <div class="dialog-body">
            ...
            <div v-if="form.authType === 'bearer'" class="field">
              ...
              <input v-model="form.token" class="field-input" placeholder="Bearer token 值（留空表示不修改）" type="password" />
            </div>
            ...
            <template v-if="form.authType === 'header'">
              ...
              <div class="field">
                ...
                <input v-model="form.headerValue" class="field-input" placeholder="Header 值（留空表示不修改）" type="password" />
              </div>
            </template>
...
watch(() => props.visible, (v) => {
  if (v && props.mcp) {
    form.value = {
      url: props.mcp.url || '',
      authType: props.mcp.authType || 'bearer',
      token: '',
      headerName: props.mcp.headerName || '',
      headerValue: '',
    }
...
watch(() => [form.value.url, form.value.authType, form.value.token], () => {
...
async function handleSave() {
  ...
  const updates = {
    ...
    ...(form.value.headerValue ? { headerValue: form.value.headerValue } : {}),
